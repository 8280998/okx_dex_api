...
DEFAULT_TOKEN_ADDRESS = Web3.to_checksum_address('0xd9aAEc86B65D86f6A7B5B1b0c42FFA531710b6CA')  # USDbC
...
wait_account_min = 10
wait_account_max = 20
gas_limit = 800000  # 默认gas限制
tx_timeout = 10  # 默认交易确认超时（秒）
max_retries = 3  # 默认重试次数
is_running = False
current_token_address = DEFAULT_TOKEN_ADDRESS
current_target_address = USDC_ADDRESS
...
class TradingApp:
    ...
    def setup_config_section(self, frame):
        ...
        # 第三组参数 - Gas限制
        ttk.Label(params_frame, text="Gas限制:").pack(side='left', padx=(15, 2))
        self.gas_limit_entry = ttk.Entry(params_frame, width=8)
        ...
        self.token_contract_entry.insert(0, DEFAULT_TOKEN_ADDRESS)
        ...
        self.target_contract_entry.insert(0, USDC_ADDRESS)
    ...
    def load_config(self):
        try:
            if os.path.exists('config.json'):
                ...
                self.gas_limit_entry.insert(0, str(config.get('gas_limit', 350000)))
                ...
                self.token_contract_entry.insert(0, config.get('token_contract', DEFAULT_TOKEN_ADDRESS))
                self.target_contract_entry.delete(0, tk.END)
                self.target_contract_entry.insert(0, config.get('target_contract', USDC_ADDRESS))
    ...
    def save_config(self):
        try:
            config = {
                ...
                'gas_limit': int(float(self.gas_limit_entry.get())),
...
def get_quote(params):
    try:
        api_url = 'https://web3.okx.com/api/v5/dex/aggregator/quote'
        timestamp = datetime.datetime.utcnow().isoformat(timespec='milliseconds') + 'Z'
        query_string = urllib.parse.urlencode(params)
        request_path = '/api/v5/dex/aggregator/quote?' + query_string
...
def get_swap_data(params):
    try:
        api_url = 'https://web3.okx.com/api/v5/dex/aggregator/swap'
        timestamp = datetime.datetime.utcnow().isoformat(timespec='milliseconds') + 'Z'
        query_string = urllib.parse.urlencode(params)
        request_path = '/api/v5/dex/aggregator/swap?' + query_string
        message = timestamp + 'GET' + request_path
        sign = base64.b64encode(hmac.new(okx_secret.encode(), message.encode(), hashlib.sha256).digest()).decode()
        headers = {
            'OK-ACCESS-KEY': okx_api_key,
...
def get_approve_data(chain_id, token_address, approve_amount='115792089237316195423570985008687907853269984665640564039457584007913129639935'):
    try:
        api_url = 'https://web3.okx.com/api/v5/dex/aggregator/approve-transaction'
        params = {
            'chainId': chain_id,
        ...
        timestamp = datetime.datetime.utcnow().isoformat(timespec='milliseconds') + 'Z'
        query_string = urllib.parse.urlencode(params)
        request_path = '/api/v5/dex/aggregator/approve-transaction?' + query_string
        message = timestamp + 'GET' + request_path
        sign = base64.b64encode(hmac.new(okx_secret.encode(), message.encode(), hashlib.sha256).digest()).decode()
        headers = {
            'OK-ACCESS-KEY': okx_api_key,
...
def approve_token(token_address, account, spender, log_callback=None):
    try:
        ...
        # 获取授权数据
        approve_resp = get_approve_data('8453', token_address)
...
def main(log_callback=None, update_stats_callback=None, refresh_stats_callback=None):
    global current_token_address, current_target_address, is_running
